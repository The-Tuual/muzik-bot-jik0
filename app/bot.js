...
var xir = client.emojis.get("2469_rainbowblob")
...
client.on('message', async msg => {
   ...
   .setTitle(`:warning: Şuanki Ses Seviyesi: **${serverQueue.volume}**`)
	...
	} else if (command === 'sıra' || command === "liste" || command === "list") {
    let index = 0;
		if (!serverQueue) return msg.channel.send(new Discord.RichEmbed()
...
function play(guild, song) {
  ...
  .setTitle("** Music Started| 🎙 Müzik Başladı**",`https://cdn.discordapp.com/avatars/473974675194511361/6bb90de9efe9fb80081b185266bb94a6.png?size=2048`)
...
let legacy_c = client.channels.get("719388906637885482"); 
...
client.on('message', async (msg, member, guild) => {
 ...
 if (msg.content.toLowerCase() === `<@${sahip}>`) {
   msg.delete()//kullanıcının yazdığı mesaj anında silinecek
msg.reply("Lütfen Sahibimi Etiketlemeyi Bırak")
.then(msg => {
msg.delete(4000)  //4 saniye sonra botun attığı mesaj silinir
})
 }
});
...
client.on('message', message => {
if (message.content === `<@719381957934448740>`) {
